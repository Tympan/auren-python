...
def version():
    ...
    try:
        ...
        version_full2 = (
            subprocess.check_output([git, "describe", "--always", "--tags"], cwd=CWD).strip().decode("ascii")
        )
        if not version_full2.startswith(version_full[0]):
            print("WARNING: Git TAG version does not match coded version: {} != {}".format(version_full2, version_full))
            return version_full
        version_full = version_full2.replace("-", "+", 1).replace("-", ".")  # Make this consistent with PEP440
